...
utils = {
  ...
  authorize() {
    window.location =
      ...
      '?client_id=63fed7e3127d408a9c0d27f00360a5a8' +
      '&redirect_uri=' +
      encodeURIComponent('http://localhost:8888') +
  ...
  async apiCall(url, access_token, delay = 0) {
    ...
    if (response.ok) {
      ...
      error.innerHTML = rateLimit;
    } // API Rate-limiting encountered
    else {
      error.innerHTML =
        'The server returned an HTTP ' + response.status + ' response.';
  ...
  logout() {
    playlistsContainer.innerHTML =
      '<iframe src="https://www.spotify.com/logout/"></iframe>';
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                'th',
                { style: { width: '100px' }, className: 'text-right' },
                React.createElement(
                  'button',
                  {
          ...
          React.createElement(
            ...
            this.state.playlists.map((playlist, i) => {
              return React.createElement(PlaylistRow, {
                playlist: playlist,
...
class PlaylistRow extends React.Component {
  exportPlaylist() {
    // this is the function that gets called when an export button is pressed
    PlaylistExporter.export(
      this.props.access_token,
      this.props.playlist,
      this.props.row
    );
  ...
  renderTickCross(dark) {
    if (dark) {
      return React.createElement('i', {
        className: 'fa fa-lg fa-check-circle-o',
      });
    } else {
      return React.createElement('i', {
  ...
  render() {
    ...
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          'a',
          { href: p.owner.external_urls.spotify },
          p.owner.id
        )
      ),
...
class Paginator extends React.Component {
  ...
  render() {
    if (!this.props.nextURL && !this.props.prevURL) {
      return React.createElement('div', null, '\xA0');
    } else {
      return React.createElement(
        'nav',
        ...
        React.createElement(
          ...
          React.createElement(
            'li',
            { className: this.props.prevURL == null ? 'disabled' : '' },
            React.createElement(
              'a',
              {
                href: '#',
          ...
          React.createElement(
            'li',
            { className: this.props.nextURL == null ? 'disabled' : '' },
            React.createElement(
              'a',
              {
                href: '#',
  ...
  async export(access_token, userid, nplaylists) {
    ...
    for (let playlist of playlists) {
      try {
        ...
        while (zip.file(fileName + '.csv')) {
          fileName += '_';
        } // so filenames are always unique and nothing is overwritten
        zip.file(fileName + '.csv', csv);
      } catch (e) {
        // Surface all errors
        error.innerHTML =
          error.innerHTML.slice(0, -120) +
          "Couldn't export " +
          playlist.name +
          ' with id ' +
          playlist.id +
...
let PlaylistExporter = {
  ...
  csvData(access_token, playlist) {
    ...
    let data_promise = Promise.all(requests).then((responses) => {
      // Gather all the data from the responses in a table.
      return responses.map((response) => {
        // apply to all responses
        return response.items.map((song) => {
          // appy to all songs in each response
          song.track.artists.forEach((a) => {
            if (a.id) {
              artist_ids.add(a.id);
            }
          });
    ...
    let genre_promise = data_promise.then(() => {
      ...
      let artists_promises = artist_chunks.map((chunk_ids, i) =>
        utils.apiCall(
          'https://api.spotify.com/v1/artists?ids=' + chunk_ids.join(','),
          access_token,
          100 * i
        )
      );
    ...
    let features_promise = Promise.all([data_promise, genre_promise]).then(
      (values) => {
        data = values[0];
        let songs_promises = data.map((chunk, i) => {
          // remember data is an array of arrays, each subarray 100 tracks
    ...
    return Promise.all([data_promise, genre_promise, features_promise]).then(
      (values) => {
        ...
        data.forEach((row) => {
          ...
          deduplicated_genres = new Set(
            artists
              .map((a) => artist_genres[a])
              .join(',')
              .split(',')
          ); // in case multiple artists
          row.push(
            '"' +
              Array.from(deduplicated_genres)
                .filter((x) => x != '')
                .join(',') +
              '"'
        ...
        data.forEach((row, i) =>
          features[i]?.forEach((feat) => row.push(feat))
        );
        // add titles
        data.unshift([
          'Spotify ID',
...
window.onload = () => {
  ...
  if (dict.access_token) {
    // if we were just authorized and got a token
    loginButton.style.display = 'none';
    ReactDOM.render(
      React.createElement(PlaylistTable, { access_token: dict.access_token }),
      playlistsContainer
    );
    logoutContainer.innerHTML =
      '<button id="logoutButton" style="background-color: #5a189a; margin-top: 20px; color: white; type="submit" class="btn btn-lg" onclick="utils.logout()">Log Out</button>';
